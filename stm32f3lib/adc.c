...
void ADC_TempSensorCmd(ADC_TypeDef* ADCx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  ...
  else
   {
     ...
     ADC1_2_COMMON->CCR &= ~(uint32_t)ADC12_CCR_TSEN;
   }
}
...
void ADC_VrefintCmd(ADC_TypeDef* ADCx, FunctionalState NewState)
{
  if((ADCx == ADC1) || (ADCx == ADC2))
  {
    ...
    else
    {
      ...
      ADC1_2_COMMON->CCR &= ~(uint32_t)ADC12_CCR_VREFEN;
    }
  }
#ifdef ADC3_4
  else
  ...
  if (NewState != DISABLE)
   {
     ...
     ADC1_2_COMMON->CCR |= ADC12_CCR_VBATEN;
   }
  else
   {
     /* Disable the Vbat channel*/
     ADC1_2_COMMON->CCR &= ~(uint32_t)ADC12_CCR_VBATEN;
   }
}
...
void ADC_ClearCommonFlag(ADC_TypeDef* ADCx, uint32_t ADC_FLAG)
{
  ...
  {
    /* Clear the selected ADC flags */
    ADC1_2_COMMON->CSR |= (uint32_t)ADC_FLAG;
  }
#ifdef ADC3_4
  else
  {
