...
int tlv_init(void)
{
  ...
  if (rc)
    return rc;
  rc = tlv320dac3100_set_clock(TLV320DAC3100_PLL_CLKIN_MCLK, TLV320DAC3100_CODEC_CLKIN_MCLK);
  if (rc)
    return rc;
  rc = tlv320dac3100_set_ndac(1, 1);
  if (rc)
    return rc;
  ...
  if (rc)
    return rc;
  //pll_clk = 4096KHz * 20
  //rc = tlv320dac3100_set_pll(1, 1, 1, 20, 0);
  //if (rc)
  //  return rc;
